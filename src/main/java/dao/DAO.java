...
public class DAO implements IDAO {
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie käyttäjätunnuksen tietokannasta haeTili
	 * metodilla.
	 * 
	 * @param tunnus
	 * 
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie käyttäjän ibanin tietokannasta
	 * haeTiliIBAN metodilla.
	 * 
	 * @param iban
	 * 
	 */
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie asiakkaan tietokannasta haeAsiakas
	 * metodilla.
	 * 
	 * @param asiakasID
	 * 
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie maksutapahtuman tietokannasta
	 * haeMaksutapahtuma metodilla.
	 * 
	 * @param tapahtumaID
	 * 
	 */
	...
	/**
	 * 
	 * luo istuntotehtaan sekä luo uuden maksutapahtuman tietokantaan
	 * luoMaksutapahtuma metodilla.
	 * 
	...
	/**
	 * 
	 * luo istuntotehtaan sekä muuttaa tietokannassa olevaa saldoa muutaSaldo
	 * metodilla.
	 * 
	 * @param iban
	 * @param saldo
	...
	/**
	 * 
	 * luo istuntotehtaan sekä muuttaa tietokannassa olevia kryptovaluuttojen saldoa
	 * muutaKryptoSaldo metodilla.
	 * 
	 * @param iban
	 * @param saldo
	 * @param tunnus
	...
	/**
	 * 
	 * luo istuntotehtaan sekä muuttaa tietokannassa olevaa tilin lainamäärää
	 * muutaLainaVelka metodilla.
	 * 
	 * @param iban
	 * @param laina
	 * 
	...
	/**
	 * 
	 * luo istuntotehtaan sekä poistaa maksutapahtuman tietokannasta
	 * poistaMaksutapahtuma metodilla.
	 * 
	 * @param tapahtumaID
	 * 
	 */
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie maksuhistorian tietokannasta
	 * ArrayList:iin haeMaksuhistoria metodilla.
	 * 
	 * @param iban
	 * 
	 */
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie odottavat maksutapahtumat tietokannasta
	 * ArrayList:iin haeOdottavatMaksut metodilla.
	 * 
	 * @param iban
	 * @param pvm
	 * 
	...
	public ArrayList<Maksutapahtuma> haeOdottavatMaksut(String iban, LocalDate pvm) {
		...
		try {
			istunto = istuntotehdas.openSession();
			istunto.beginTransaction();
			String query = "FROM Maksutapahtuma WHERE vastaanottajaiban = '" + iban
					+ "' AND tila = 'F' AND ajankohta BETWEEN '0001-01-01' AND '" + pvm + "'";
			lista = (ArrayList<Maksutapahtuma>) istunto.createQuery(query).getResultList();
			istunto.getTransaction().commit();
			System.out.println();
		} catch (Exception e) {
	...
	/**
	 * 
	 * luo istuntotehtaan sekä merkkaa maksutapahtuman maksetuksi tietokantaan
	 * merkkaaMaksetuksi metodilla.
	 * 
	 * @param ID
	 * 
	 */
	...
	/**
	 * 
	 * luo istuntotehtaan sekä muuttaa tietokannassa olevaa käyttäjän sähköpostia
	 * muutaSposti metodilla.
	 * 
	 * @param sposti
	 * 
	 */
	...
	public void muutaSposti(String sposti) {
		...
		try (Session istunto = istuntotehdas.openSession()) {
			transaktio = istunto.beginTransaction();
			String query = "update Asiakas set sahkoposti = '" + sposti + "' where asiakas_id = '"
					+ AktiivinenKayttaja.getInstance().getAsiakastiedot().getId() + "'";
			istunto.createQuery(query).executeUpdate();
			transaktio.commit();
		} catch (Exception e) {
			if (transaktio != null)
	...
	/**
	 * 
	 * luo istuntotehtaan sekä muuttaa tietokannassa olevaa käyttäjän puhelinnumeroa
	 * muutaPuh metodilla.
	 * 
	 * @param puh
	 * 
	...
	public void muutaPuh(String puh) {
		...
		try (Session istunto = istuntotehdas.openSession()) {
			transaktio = istunto.beginTransaction();
			String query = "update Asiakas set puhelin = '" + puh + "' where asiakas_id = '"
					+ AktiivinenKayttaja.getInstance().getAsiakastiedot().getId() + "'";
			istunto.createQuery(query).executeUpdate();
			transaktio.commit();
		} catch (Exception e) {
	...
	/**
	 * 
	 * luo istuntotehtaan sekä muuttaa tietokannassa olevaa käyttäjän lähiosoitetta
	 * muutaOsoite metodilla.
	 * 
	 * @param osoite
	 * 
	 */
	...
	public void muutaOsoite(String osoite) {
		...
		try (Session istunto = istuntotehdas.openSession()) {
			transaktio = istunto.beginTransaction();
			String query = "update Osoite set lahiosoite = '" + osoite + "' where osoite_id = "
					+ AktiivinenKayttaja.getInstance().getOsoite().getOsoite_id();
			istunto.createQuery(query).executeUpdate();
			transaktio.commit();
		} catch (Exception e) {
			if (transaktio != null)
	...
	/**
	 * 
	 * luo istuntotehtaan sekä muuttaa tietokannassa olevaa käyttäjän postinumeroa
	 * muutaPostiNro metodilla.
	 * 
	 * @param postinro
	 * 
	 */
	...
	public void muutaPostiNro(String postinro) {
		...
		try (Session istunto = istuntotehdas.openSession()) {
			transaktio = istunto.beginTransaction();
			// TODO pls fix (aka add kaikki osoitteet), testaa tietokannassa olevilla
			// postinumeroilla
			String query = "update Osoite set postinro = '" + postinro + "' where osoite_id = "
					+ AktiivinenKayttaja.getInstance().getOsoite().getOsoite_id();
			istunto.createQuery(query).executeUpdate();
			transaktio.commit();
		} catch (Exception e) {
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie valuutat tietokannasta ArrayList:iin
	 * haeValuutat metodilla.
	 * 
	 */
	...
	/**
	 * 
	 * luo istuntotehtaan sekä päivittää valuuttakurssin tietokantaan
	 * paivitaValuutta metodilla.
	 * 
	 * @param valuutta
	 * 
	 */
	...
	public void paivitaValuutta(Valuutta valuutta) {
		...
		try (Session istunto = istuntotehdas.openSession()) {
			transaktio = istunto.beginTransaction();
			String query = "update Valuutta set valuuttakurssi = " + valuutta.getKurssi() + " where valuutta_id = '"
					+ valuutta.getTunnus() + "'";
			istunto.createQuery(query).executeUpdate();
			transaktio.commit();
		} catch (Exception e) {
	...
	/**
	 * 
	 * luo istuntotehtaan sekä hakee ja vie maksupohjat tietokannasta ArrayList:iin
	 * haeMaksupohjat metodilla.
	 * 
	 * @param iban
	 * 
	 */
	...
	/**
	 * 
	 * luo istuntotehtaan sekä poistaa maksupohjan tietokannasta poistaMaksupohja
	 * metodilla.
	 * 
	 * @param maksupohja_id
	 * 
