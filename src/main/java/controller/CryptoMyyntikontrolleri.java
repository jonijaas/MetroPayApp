...
/**
 * CryptoMyyntikontrollerissa on kaikki krypton myynti näkymään liittyvät
 * metodit sekä toiminnot.
 * 
 * @author Ryhmä 7
 *
 * 
...
public class CryptoMyyntikontrolleri {
	...
	/**
	 * 
	 * myynti() metodi hoitaa kryptojen myynti ominaisuudet. Metodi laskee
	 * lisättävän ja poistettavan summan tilille.
	 * 
	 * @param e
	 * @throws IOException
	 * 
	...
	public void myynti(MouseEvent e) throws IOException {
		try {
			if (valuuttaBox.getSelectionModel().getSelectedItem().getTunnus().equals("BTC")) {
				...
				double muutettuSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldo()
						+ (Double.parseDouble(maaraTxt.getText()) / vDao.haeValuutta("BTC"));
				double muutettuKryptoSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldoBtc()
						- Double.parseDouble(maaraTxt.getText());
				...
				AktiivinenKayttaja.getInstance().asetaMaksu(
						new Maksutapahtuma(null, Double.parseDouble(maaraTxt.getText()), "₿ ", null, null, null, null, null, null));
				...
				double muutettuSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldo()
						+ (Double.parseDouble(maaraTxt.getText()) / vDao.haeValuutta("DOGE"));
				double muutettuDogeSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldoDoge()
						- Double.parseDouble(maaraTxt.getText());
				if (Double.parseDouble(maaraTxt.getText()) < 0) {
					System.out.println("Negatiivinen luku");
					throw new Exception();
				...
				AktiivinenKayttaja.getInstance().setTili(dao.haeTili(AktiivinenKayttaja.getInstance().getTili().getTunnus()));
				AktiivinenKayttaja.getInstance().asetaMaksu(
						new Maksutapahtuma(null, Double.parseDouble(maaraTxt.getText()), "Ð ", null, null, null, null, null, null));
				gui.vaihdaNäkymä("kryptoMyyntivahvistaminen.fxml");

			} else if (valuuttaBox.getSelectionModel().getSelectedItem().getTunnus().equals("ADA")) {

				double muutettuSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldo()
						+ (Double.parseDouble(maaraTxt.getText()) / vDao.haeValuutta("ADA"));
				double muutettuAdaSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldoAda()
						- Double.parseDouble(maaraTxt.getText());
				if (Double.parseDouble(maaraTxt.getText()) < 0) {
					System.out.println("Negatiivinen luku");
					throw new Exception();
				...
				double muutettuSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldo()
						+ (Double.parseDouble(maaraTxt.getText()) / vDao.haeValuutta("ETH"));
				double muutettuEthSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldoEth()
						- Double.parseDouble(maaraTxt.getText());
				if (Double.parseDouble(maaraTxt.getText()) < 0) {
					System.out.println("Negatiivinen luku");
					throw new Exception();
				...
				AktiivinenKayttaja.getInstance().setTili(dao.haeTili(AktiivinenKayttaja.getInstance().getTili().getTunnus()));
				AktiivinenKayttaja.getInstance().asetaMaksu(
						new Maksutapahtuma(null, Double.parseDouble(maaraTxt.getText()), "Ξ ", null, null, null, null, null, null));
				gui.vaihdaNäkymä("kryptoMyyntivahvistaminen.fxml");
			} else {
	...
	/**
	 * initialize metodi ajetaan heti näkymän avautumassa. Metodissa täytetään
	 * valuutta lista ja asetetaan siihen OnActin metodi
	 */
