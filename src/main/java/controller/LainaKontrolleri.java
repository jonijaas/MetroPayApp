...
/**
 * 
 * LainaKontrollerissa on toteutkset lainalle, sen laskemiselle ja lainasivun
 * päivittämisen oikeilla tiedoilla
 * 
 * @author Ryhmä 7
 * 
 *
...
public class LainaKontrolleri implements IKontrolleri {
	...
	/**
	 * asetaSliderArvo() metodi säätää lainanäkymän slideria. Metodi päivittää
	 * sliderin niin että se nousee tai laskee vähintään 50€
	 */
	...
	/**
	 * asetaSliderArvo() metodi laskee ja päivittää lainan kuukautisen erän
	 * näkymässä.
	 */
	...
	public void kasitteleLaina() throws IOException {
		...
		if (vanhalaina + uusilaina <= 2000) {
			...
			AktiivinenKayttaja.getInstance()
					.asetaMaksu(new Maksutapahtuma(null, korkoBox.getSelectionModel().getSelectedItem(), maaraTxt.getText(),
							kuukausiEraLabel.getText(), null, null, null, null, null));
	...
	/**
	 * initialize metodi ajetaan heti näkymän avautumassa. katsotaan onko
	 * käyttäjällä aikaisempia lainoja ja rajoitetaan lainan määrä sen mukaan.
	 */
	@FXML
	void initialize() {
		...
		if (vanhalaina > 0) {
			vanhaLainaMaaraLabel.setText(numberFormat.format(vanhalaina) + " €");
		} else {
