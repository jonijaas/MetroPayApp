...
/**
 * CryptoMyyntikontrollerissa on kaikki krypton osto näkymään liittyvät metodit
 * sekä toiminnot.
 * 
 * * @author Ryhmä 7
 *
...
public class CryptoOstokontrolleri {
	...
	/**
	 * osta() metodi hoitaa kryptojen osto ominaisuudet. Metodi laskee lisättävän ja
	 * poistettavan summan tilille.
	 * 
	 * @param e
	 * @throws IOException
	 */
	public void osta(MouseEvent e) throws IOException {
		...
		try {
			...
			System.out
					.println(AktiivinenKayttaja.getInstance().getTili().getSaldo() - Double.parseDouble(maaraTxt.getText()));
			...
			if (valuuttaBox.getSelectionModel().getSelectedItem().getTunnus().equals("BTC")) {
				...
				double muutettuBtcSaldo = AktiivinenKayttaja.getInstance().getTili().getSaldoBtc()
				...
				AktiivinenKayttaja.getInstance().asetaMaksu(new Maksutapahtuma(0, null, Double.parseDouble(maaraTxt.getText()),
