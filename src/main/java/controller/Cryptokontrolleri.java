...
/**
 * Cryptokontrollerissa on kaikki krypto näkymään liittyvät metodit sekä
 * toiminnot.
 * 
 * @author Ryhmä
 *
...
public class Cryptokontrolleri implements Initializable {
	...
	/**
	 * chartInit luo graafin, joka näyttää missä valitun kryptovaluutan kurssi
	 * menee.
	 * chartInit tekee paljon kutsuja CryptoChartModel luokkaan, joka taas kutsuu
	 * cryptocompare apia.
	 * 
	 * @param valuutta
	 * 
	...
	public void chartInit(String valuutta) {
		...
		// 1582 1611 1569ms when below is done sync significantly slower vs 466 482
		// 480ms with no sync
		...
		CompletableFuture<Void> addSeriesData = CompletableFuture
				.runAsync(() -> CryptoChartModel.addSeriesData(seriesMinute)).thenRun(() -> {
					CryptoChartModel.addSeriesDataHour(seriesHour);
				}).thenRun(() -> {
					CryptoChartModel.addSeriesDataDay(seriesDay);
				});
	 ...
	 * asetaSaldo(ja haeSaldo()) metodeja, jotka alustavat valuutan näytön.
	 * 
	 */
